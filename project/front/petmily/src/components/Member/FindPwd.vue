<template>
  <div class="findpwd_area">
    ...
    <span class="findid" v-show="findid"
      >아이디가 기억나지 않는다면?
      <span class="findidlink" @click="findId">아이디 찾기</span></span
    >
    ...
    <div class="input_box" v-show="emailCodeShow">
      <input
        type="text"
        class="input_code_txt"
        id="input_code_txt"
        v-model="emailCode"
        placeholder="인증코드"
      />
      <button class="input_button" v-on:click="emailCodeCheck">인증</button
      ><br />
    </div>

    <div class="input_box" v-show="changepwdShow">
      <p>본인 인증절차가 완료되었습니다.</p>
      <p>새로운 비밀번호를 재설정해주시기 바랍니다.</p>
      <br />
      <input
        type="password"
        class="input_txt"
        ...
        placeholder="PWD"
      /><br />
      <span class="input_error" v-show="isPwdCheck"
        >영문, 숫자, 특수문자 8~16문자</span
      ><br />
      ...
      <input
        ...
        @blur="checkPwdEqual"
      /><br />
      <span class="input_error" v-show="isPwdCheckEqual"
        >비밀번호 확인해주세요</span
      ><br />
      <button class="input_button" @click="changepwd" style="margin-top: 10px">
        변경
      </button>
    </div>
  </div>
...
export default {
  ...
  methods: {
    sendEmail() {
      ...
      self.$axios
        .get("http://localhost:8082/members/" + self.id)
        .then(function (res) {
          if (res.status == 200) {
            ...
            if (self.dto != null) {
              if (self.dto.id === self.id && self.dto.email === self.email) {
                self.$axios
                  .post("http://localhost:8082/emailConfirm", formdata)
                  .then(function (res) {
                    if (res.status == 200) {
    ...
    changepwd() {
      ...
      if (self.pwdtf) {
        alert("비밀번호 확인 완료");
        self.$axios
          .put("http://localhost:8082/members/pwd", formdata)
          .then(function (res) {
            if (res.status == 200) {
              alert("비밀번호 수정 완료");
