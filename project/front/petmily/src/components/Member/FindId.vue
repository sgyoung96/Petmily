<template>
  <div class="findid_area">
    ...
    <div class="input_box">
      <button class="input_button" v-on:click="emailcheck" :disabled="findid">
        이메일 확인</button
      ><br />
    </div>
    ...
    <div v-show="info">
      <div class="findid">
        회원님의 아이디는 [&nbsp;<span class="myid">{{ id }}</span
        >&nbsp;] 입니다.
      </div>
      <div class="findpwd">
        비밀번호가 기억나지 않는다면?
        <span class="findpwdlink" @click="findpwd">비밀번호 찾기</span>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    emailcheck() {
      ...
      self.$axios
        .get("http://localhost:8082/members/email/" + self.email)
        .then(function (res) {
          //
          if (res.status == 200) {
